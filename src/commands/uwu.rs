use poise::{serenity_prelude::{ContentSafeOptions, content_safe, Colour}, send_reply};
...
pub async fn uwu(ctx: Context<'_>, #[description = "Text to UwUify"]text:String) -> Result<(), Error> {
...
///Medium level of uwuness
  #[poise::command(slash_command, prefix_command )]
  pub async fn owo(ctx: Context<'_>, #[description = "Text to OwOify"]text:String) -> Result<(), Error> {
  uwoh(ctx,text,OwoifyLevel::Owo).await?;
      Ok(())
    }
///Maximum level of uwuness
    #[poise::command(slash_command, prefix_command )]
    pub async fn uvu(ctx: Context<'_>, #[description = "Text to UvUify"]text:String) -> Result<(), Error> {
